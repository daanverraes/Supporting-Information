...
def signed_angle_between_three_points(p1, p2, p3):
    """
    Calculate the clockwise angle between three 3D points with p2 as the vertex.

    Parameters:
        p1, p2, p3: Arrays or lists representing the 3D points [x, y, z].

    Returns:
        angle: The clockwise angle in degrees.
    """
    ...
    # Compute the cross product (a vector in 3D) and dot product
    cross = np.cross(v1, v2)  # Cross product gives a 3D vector
    dot = np.dot(v1, v2)      # Dot product is a scalar
    
    # Compute the magnitude of the cross product (scalar length of the vector)
    cross_magnitude = np.linalg.norm(cross)
    
    # Compute the angle in radians using arctan2
    angle_radians = np.arctan2(cross_magnitude, dot)
    
    # Convert the angle to degrees
    angle_degrees = np.degrees(angle_radians)
    
    # Direction of the angle: determine if it's clockwise or counterclockwise
    # Using the z-component of the cross product to check the direction
    if cross[2] < 0:  # Assuming z-axis determines the 'up' direction
